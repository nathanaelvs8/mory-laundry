        ...
        <DashboardLayout title="Buat Pesanan Baru">
            <div className="table-container" style={{ padding: '20px' }}>
                <form onSubmit={handleSubmitClick}>
                    ...
                    <div style={{overflowX: 'auto', WebkitOverflowScrolling: 'touch', marginBottom: 15}}>
                        <table style={{minWidth: 450}}>
                            <thead><tr><th>Layanan</th><th style={{ textAlign: 'center' }}>Jumlah</th><th style={{ textAlign: 'right' }}>Harga</th><th style={{ textAlign: 'right' }}>Subtotal</th><th style={{ width: 50 }}></th></tr></thead>
                            <tbody>
                                {orderItems.length === 0 ? <tr><td colSpan="5" style={{ textAlign: 'center', padding: 30, color: '#999' }}>Belum ada layanan dipilih</td></tr>
                                    : orderItems.map((item, index) => (
                                        <tr key={index}>
                                            ...
                                            <td><button type="button" className="btn btn-sm" onClick={() => handleRemoveItem(index)} style={{ background: '#fee', color: '#d00', padding: '6px 10px' }}><FaTrash /></button></td>
                                        </tr>
                                    ))}
                            </tbody>
                        </table>
                    ...
                    <div style={{ background: 'linear-gradient(135deg, var(--gold), var(--gold-light))', borderRadius: 10, padding: 15, color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                    ...
                    <div className="form-group">
                        <label className="form-label" style={{fontSize: 13}}>Alamat (opsional)</label>
                        <textarea className="form-control" rows="2" value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} placeholder="Alamat lengkap" style={{ resize: 'vertical' }}></textarea>
                    </div>
                    <div className="form-group">
                        <label className="form-label" style={{fontSize: 13}}>Catatan (opsional)</label>
                        <textarea className="form-control" rows="2" value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} placeholder="Catatan khusus" style={{ resize: 'vertical' }}></textarea>
                    </div>
            ...
            <ConfirmModal isOpen={confirmModal} onClose={() => setConfirmModal(false)} onConfirm={handleConfirmSubmit} title="Buat Pesanan?" message={`Total pesanan: Rp ${formatPrice(getTotalPrice())}. Lanjutkan?`} type="warning" />
        </DashboardLayout>
    );
};
